...
-The second dimension is the flattened input
*/
...
Tensor FlattenLayer::backward(const Tensor& dyTensor) {
	...
	std::vector<Eigen::MatrixXd> output = std::vector<Eigen::MatrixXd>(batchSize, Eigen::MatrixXd(inputChannels, inputHeight * inputWidth));
	for (int z = 0; z < batchSize; z++) {
		int matrixSize = inputHeight * inputWidth;
		for (int f = 0; f < inputChannels; f++) {

			Eigen::RowVectorXd channel = dy.row(z).segment(f * matrixSize, matrixSize);
			output[z].row(f) = channel;
		}
	}
	return Tensor::tensorWrap(output);
}
